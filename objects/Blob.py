...
class Blob:
    ...
    def handle_events(self, event, manager, round):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            if self.circle.collidepoint(event.pos[0], event.pos[1]):
                self.round = round
                if self.tk:
                    self.label2.config(text="Time Point: " + str(self.round))
                else:
                    self.tk = manager.tk
    ...
    def draw(self):
        ...
        label.place(x=0, y=0)
        self.label2 = tkinter.Label(frame, text="Time Point: " + str(self.round), bg="#333", fg="white", font=("Montserrat", 14))
    ...
    def ask(self):
        ...
        question = tkinter.Label(self.scrollable_frame,
                                 text="Question: " + selection1 + selection2.lower() + text + " at time point " + str(
                                     self.round) + "?",
